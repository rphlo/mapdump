...
const drawRoute = async (img, corners_coords, route, includeHeader=false, includeRoute=true, tz='Europe/Helsinki') => {
  ...
  if (includeHeader) {
    ...
    if (includeRoute && route.length && route[0].time) {
      ...
      ctx4.fillText(`${(new Date(route[0].time)).toLocaleString(undefined, {timeZone: tz, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', timeZoneName: 'long', hour12: false, hour: 'numeric', minute:'numeric', second:'numeric'})}`, paletteX + paletteWidth + 30, paletteY + 15);
    }
