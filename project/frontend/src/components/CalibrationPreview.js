...
const CalibrationPreview = (props) => {
  ...
  useEffect(() => {
    ...
    const transformedImage = L.imageTransform(imgDataURI, cornersCoordinates, {opacity: 0.7});
      ...
      <>
        <div>
            <div className="row">
                <div className="col-md-12">
                    <div className="alert alert-info" role="alert">
                        <span id="help_text">Check that your map is well aligned with the world map.</span>
                    </div>
                </div>
            </div>
            <div className="row">
            ...
            <div className="row" style={{marginTop: "10px"}}>
                <div className="col-md-12">
                <button className="btn btn-danger" onClick={() => {onValue(false)}}><i className="fas fa-undo"></i> Back</button>&nbsp;
                <button className="btn btn-primary" onClick={() => onValue(getCalibrationString(cornersCoordinates))}><i className="fas fa-arrow-alt-circle-right"></i> Validate Calibration</button>
                </div>
            </div>
        </div>
    </>
